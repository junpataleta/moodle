...
class provider implements metadataprovider, \mod_assign\privacy\assignsubmission_provider {
    ...
    public static function delete_submission_for_userid(submission_request_data $exportdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($exportdata->get_context()->id, 'assignsubmission_file', ASSIGNSUBMISSION_FILE_FILEAREA,
                $submissionid);

        $DB->delete_records('assignsubmission_file', ['assignment' => $exportdata->get_assign()->get_instance()->id,
