...
class provider implements metadataprovider, assignfeedback_provider {
     ...
     */
    public static function export_feedback_user_data(feedback_request_data $exportdata) {
        $currentpath = $exportdata->get_subcontext();
        $currentpath[] = get_string('privacy:path', 'assignfeedback_editpdf');
        $plugin = $exportdata->get_subplugin();
        $fileareas = $plugin->get_file_areas();
        $grade = $exportdata->get_grade();
     ...
     */
    public static function delete_feedback_for_context(feedback_request_data $requestdata) {
     ...
     */
    public static function delete_feedback_for_grade(feedback_request_data $requestdata) {
        global $DB;
        ...
        foreach ($fileareas as $filearea => $notused) {
            ...
            $fs->delete_area_files($requestdata->get_context()->id, 'assignfeedback_editpdf',
                    $filearea, $requestdata->get_grade()->id);
        }

        // Remove table entries.
        $DB->delete_records('assignfeedback_editpdf_annot', ['gradeid' => $requestdata->get_grade()->id]);
        $DB->delete_records('assignfeedback_editpdf_cmnt', ['gradeid' => $requestdata->get_grade()->id]);
        // Submission records in assignfeedback_editpdf_queue will be cleaned up in a scheduled task
        // (mod/assign/feedback/editpdf/classes/task/convert_submissions.php) Otherwise I need to do a select to find the
        // submission id related to this grade.
    }
}
