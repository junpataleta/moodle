...
/**
 * Adhoc task that handles the setting of event priority for existing mod_lesson calendar events for user and group overrides.
 *
 * @package    mod_lesson
...
/**
 * Class that handles the setting of event priority for existing mod_lesson calendar events for user and group overrides.
 *
 * @package     mod_lesson
 * @copyright   2017 Jun Pataleta
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class set_calendar_event_override_priorities extends adhoc_task {
    ...
    /**
     * Run the task and set the priorities of mod_lesson user and group override calendar events.
     */
    public function execute() {
        ...
        $lessonsrs = $DB->get_recordset_sql($sql);
        foreach ($lessonsrs as $record) {
