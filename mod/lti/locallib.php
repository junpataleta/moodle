 ...
 */
function lti_build_content_item_selection_request($id, $course, moodle_url $returnurl, $title = '', $mediatypes = [],
    ...
    $requestparams['accept_copy_advice'] = $copyadvice ? 'true' : 'false';
    $requestparams['accept_multiple'] = $multiple ? 'true' : 'false';
    $requestparams['accept_unsigned'] = $unsigned ? 'true' : 'false';
    $requestparams['auto_create'] = $autocreate ? 'true' : 'false';
    $requestparams['can_confirm'] = $canconfirm ? 'true' : 'false';
    ...
    // Remove parameters that should not be passed.
    $unwantedparams = [
        'resource_link_id',
        'resource_link_title',
...
/**
 * Process tool registration from the response to the Content-Item Message request.
 *
 ...
 * @param string $contentitems The JSON string for the content_items parameter.
 * @param stdClass $course The course where this tool will be added into.
 * @param int $sectionid The section ID where this tool will be added into.
 * @throws Exception
 * @throws lti\OAuthException
 */
function lti_tool_registration_from_content_item($typeid, $messagetype, $ltiversion, $consumerkey, $contentitems, $course,
                                                 $sectionid) {
    global $DB;

    // Check lti_message_type. Show debugging if it's not set to ContentItemSelectionRequest.
    ...
    if ($islti2) {
        if ($ltiversion !== LTI_VERSION_2) {
            debugging("lti_version from response does not match the tool's configuration. Tool: " .
                LTI_VERSION_2 . ", Response: {$ltiversion}", DEBUG_DEVELOPER);
        }
    } else if ($ltiversion !== LTI_VERSION_1) {
        debugging("lti_version from response does not match the tool's configuration. Tool: " .
            LTI_VERSION_1 . ", Response: {$ltiversion}", DEBUG_DEVELOPER);
    }
    ...
    $items = json_decode($contentitems);
    if (empty($items)) {
        throw new Exception('The content_items data is invalid: ' . $contentitems);
    }
    if ($items->{'@context'} !== 'http://purl.imsglobal.org/ctx/lti/v1/ContentItem') {
        throw new Exception('The Content-Item context is invalid: ' . $items->{'@context'});
    }
    if (!isset($items->{'@graph'}) || !is_array($items->{'@graph'}) || (count($items->{'@graph'}) > 1)) {
        throw new Exception('Invalid Content-Item response format.');
    }
    ...
    if (!empty($items->{'@graph'})) {
        foreach ($items->{'@graph'} as $item) {
            ...
            if (isset($item->custom)) {
                ...
                $moduleinfo->instructorcustomparameters = implode("\n", $customparameters);
            }
