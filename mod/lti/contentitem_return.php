...
if (empty($errormsg)) {
    try {
        ...
        $redirecturl = course_get_url($course, $sectionid, array('sr' => $sectionreturn));
    } catch (Exception $e) {
...
$title = get_string('autoaddtype', 'lti');
$PAGE->set_heading($title);
echo $OUTPUT->header();

// Show error or message notification.
if ($errormsg) {
    ...
    if ($msg) {
        ...
        \core\notification::info(get_string('contentitemtoolregistrationsuccess', 'lti'));
    }

    // Call JS module to redirect the user to the course page.
    if ($redirecturl) {
        $PAGE->requires->js_call_amd('mod_lti/contentitem_return', 'init', [$redirecturl->out()]);
    }
}
