...
(function(){
    ...
    M.mod_lti.editor = {
        init: function(yui3, settings){
            ...
            var configureButton = Y.one('[name="configurefromlink"]');
            var typeSelector = Y.one('#id_typeid');

            // Enables/disables the Configure from link button depending on whether the selected tool type supports the
            // Content-Item message or not.
            var updateConfigureButtonState = function() {
                var selected = self.getSelectedToolTypeOption();
                if (selected.getAttribute('data-contentitem')) {
                    configureButton.removeAttribute('disabled');
                    configureButton.setAttribute('data-contentitemurl', selected.getAttribute('data-contentitemurl'));
                } else {
                    if (!configureButton.getAttribute('disabled')) {
                        configureButton.setAttribute('disabled', 'disabled');
                        configureButton.removeAttribute('data-contentitemurl');
                    }
                }
            };
            ...
            typeSelector.on('change', function(e){
                ...
                // Update the Configure from link button's state on load, enabling/disabling when necessary.
                updateConfigureButtonState();
            });

            // Handle configure from link button click.
            configureButton.on('click', function() {
                if (configureButton.getAttribute('data-contentitemurl')) {
                    var url = configureButton.getAttribute('data-contentitemurl');
