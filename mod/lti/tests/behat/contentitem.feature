...
Feature: Content-Item support
  ...
  @javascript
  Scenario: Tool that supports Content-Item message type should be able to configure a tool via the Configure tool from link button
    When I log in as "teacher1"
    And I follow "Course 1"
    And I turn editing mode on
    And I add a "Teaching Tool 1" to section "1"
    Then the "Configure tool from link" "button" should be enabled
    And I press "Configure tool from link"
    And I should see "Configure item"
  ...
  Scenario: Adding a preconfigured tool that does not support Content-Item.
    ...
    And I turn editing mode on
    And I add a "Teaching Tool 1" to section "1"
    And the "Configure tool from link" "button" should be enabled
    ...
    # When editing settings, the Configure tool from link button should be disabled.
    And the "Configure tool from link" "button" should be disabled
