...
class activity_custom_completion_test extends advanced_testcase {
    ...
    public function test_get_overall_completion_state(array $rules, array $rulestates, int $invokecount, int $state) {
        ...
        // Mock \core_completion\activity_custom_completion::get_available_custom_rules().
        $stub->expects($this->once())
            ->method('get_available_custom_rules')
            ->willReturn($rules);

        // Mock \core_completion\activity_custom_completion::get_state().
        if ($invokecount > 0) {
    ...
    public function test_validate_rule(bool $defined, bool $available, ?string $expectedexception) {
        ...
        // Mock \core_completion\activity_custom_completion::is_defined().
        $stub->expects($this->any())
            ->method('is_defined')
            ->willReturn($defined);

        // Mock \core_completion\activity_custom_completion::is_available().
        $stub->expects($this->any())
            ->method('is_available')
