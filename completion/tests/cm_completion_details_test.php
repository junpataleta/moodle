...
class cm_completion_details_test extends advanced_testcase {
     ...
     */
    protected function setup_data(?int $completion = null, array $completionoptions = []): cm_completion_details {
        $this->resetAfterTest();
        ...
        // Need to build a proper cm_info object which is being used by the methods in cm_completion details,
        ...
        $cminfo = get_coursemodule_from_instance('assign', $assign->id);
        $cminfo = cm_info::create($cminfo);

        return new cm_completion_details($this->completioninfo, $cminfo, 2);
    }
     ...
     * @param $completion
     * @param $completionview
     * @param $completiongrade
