...
Feature: Lesson skip link moves keyboard focus past the menu
  ...
  Scenario: Activating the skip navigation link moves focus past the lesson menu
    ...
    # Without the fix the target is not focusable and its id collides with the core skip target,
    # so focus does not move to the lesson content after the menu; with the fix it does.
