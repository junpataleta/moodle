...
export const setupForTarget = async(target, options = {}) => {
    ...
    //
    // TinyMCE's own windows are rendered outside of the Moodle modal's DOM, and are not aware of Moodle's
    ...
    if (modalContainer) {
        ...
        editor.windowManager.openUrl = (...args) => {
            releaseModal();
            return originalOpenUrl(...args);
        };

        // Once TinyMCE's window is closed, show the modal again and re-trap focus within it.
