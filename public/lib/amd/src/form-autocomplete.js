...
define([
    ...
    var activateSelection = function(index, state) {
        ...
        if (index > -1) {
            ...
            selectionElement.removeAttr('aria-activedescendant');
            selectionElement.removeAttr('data-active-value');
        }
    ...
    /**
     * Set the selection container ARIA attributes according to whether it has selectable items.
     *
     ...
     * @param   {Array} items
     */
    var updateSelectionRegionAccessibility = function(options, state, selectionElement, items) {
        if (items.length) {
            selectionElement.attr('aria-labelledby', state.selectionId + '-label');
            selectionElement.attr('role', 'listbox');
            selectionElement.attr('tabindex', '0');

            if (options.multiple) {
                selectionElement.attr('aria-multiselectable', 'true');
            } else {
                selectionElement.removeAttr('aria-multiselectable');
            }

            if (state.required) {
                selectionElement.attr('aria-required', 'true');
            } else {
                selectionElement.removeAttr('aria-required');
            }
            return;
        }

        selectionElement.removeAttr('aria-labelledby');
        selectionElement.removeAttr('role');
        selectionElement.removeAttr('tabindex');
        selectionElement.removeAttr('aria-multiselectable');
        selectionElement.removeAttr('aria-required');
        selectionElement.removeAttr('aria-activedescendant');
        selectionElement.removeAttr('data-active-value');
    ...
    var updateSelectionList = function(options, state, originalSelect) {
        ...
        if (!hasItemListChanged(state, items)) {
            updateSelectionRegionAccessibility(options, state, newSelection, items);
            if (items.length) {
    ...
    var activateItem = function(index, state) {
        ...
        index = wrapListIndex(index, length);
    ...
    var getNextEnabledItem = function(current, suggestions) {
        ...
        if (nextIndex > -1 && suggestions[nextIndex].getAttribute('aria-disabled')) {
    ...
    var getPreviousEnabledItem = function(current, suggestions) {
        ...
        if (previousIndex > -1 && suggestions[previousIndex].getAttribute('aria-disabled')) {
            ...
            switch (e.keyCode) {
                ...
                case KEYS.ENTER:
                    ...
                    if (selectedItem) {
                        ...
                        // Note whether the region has focus now, before it is potentially emptied. Removing the
                        // tabindex from a non-natively-focusable, focused element makes the browser blur it (to
                        // <body>), so document.activeElement can no longer be compared against the region afterwards.
                        var hadFocus = (document.activeElement === document.getElementById(state.selectionId));

                        // Unselect this item.
                        pendingPromise.resolve(deselectItem(options, state, selectedItem, originalSelect).then(function() {
                            // If that was the last selection, the region loses its listbox role and tabindex, and
                            // focus is lost (or left stranded). Move focus to the input instead.
                            var region = document.getElementById(state.selectionId);
                            if (hadFocus && !region.hasAttribute('tabindex')) {
