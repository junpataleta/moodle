...
if ($scaleid) {
    if ($scale = $DB->get_record("scale", array('id'=>$scaleid))) {
        if ($scale->courseid == 0 || $scale->courseid == $course->id) {
            ...
            echo "<center>";
            echo html_writer::label(get_string('scales'), 'scaleunused' . $scaleid, false, ['class' => 'visually-hidden']);
...
if ($scales = $DB->get_records("scale", array("courseid"=>$course->id), "name ASC")) {
    ...
    foreach ($scales as $scale) {
        ...
        echo "<center>";
        echo html_writer::label(get_string('scales'), 'courseunused' . $scale->id, false, ['class' => 'visually-hidden']);
...
if ($scales = $DB->get_records("scale", array("courseid"=>0), "name ASC")) {
    ...
    foreach ($scales as $scale) {
        ...
        echo $OUTPUT->heading($scale->name);
        echo "<center>";
        echo html_writer::label(get_string('scales'), 'sitescale' . $scale->id, false, ['class' => 'visually-hidden']);
