...
class wsdl_test extends \advanced_testcase {
    ...
    public function test_minimum_wsdl() {
        ...
        $wsdl = new webservice_soap_wsdl($serviceclass, $namespace);
        ...
        $this->assertEquals($serviceclass . webservice_soap_wsdl::PORT, $definitions->portType->attributes()->name);

        // Test binding node and attributes.
        $this->assertNotNull($definitions->binding);
        $this->assertEquals($serviceclass . webservice_soap_wsdl::BINDING, $definitions->binding->attributes()->name);
        $this->assertEquals('tns:' . $serviceclass . webservice_soap_wsdl::PORT, $definitions->binding->attributes()->type);
        ...
        $this->assertEquals(webservice_soap_wsdl::NS_SOAP_TRANSPORT, $bindingattrs->transport);

        // Test service node.
        $this->assertNotNull($definitions->service);
        $this->assertEquals($serviceclass . webservice_soap_wsdl::SERVICE, $definitions->service->attributes()->name);

        $serviceport = $definitions->service->children()->port;
        $this->assertNotEmpty($serviceport);
        $this->assertEquals($serviceclass . webservice_soap_wsdl::PORT, $serviceport->attributes()->name);
        $this->assertEquals('tns:' . $serviceclass . webservice_soap_wsdl::BINDING, $serviceport->attributes()->binding);
    ...
    public function test_register() {
        ...
        $this->assertEquals('tns:' . $functionname . webservice_soap_wsdl::IN, $porttypeoperation->input->attributes()->message);
        $this->assertEquals('tns:' . $functionname . webservice_soap_wsdl::OUT, $porttypeoperation->output->attributes()->message);
        ...
        $this->assertEquals(webservice_soap_wsdl::NS_SOAP_ENC, $inputbody->attributes()->encodingStyle);
        ...
        $this->assertEquals(webservice_soap_wsdl::NS_SOAP_ENC, $outputbody->attributes()->encodingStyle);
        $this->assertEquals($namespace, $outputbody->attributes()->namespace);

        // Test messages.
        $messagein = $definitions->message[0];
        $this->assertEquals($functionname . webservice_soap_wsdl::IN, $messagein->attributes()->name);
        ...
        $this->assertEquals($functionname . webservice_soap_wsdl::OUT, $messageout->attributes()->name);
