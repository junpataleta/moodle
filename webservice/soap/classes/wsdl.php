...
class webservice_soap_wsdl {
    ...
    public function register($functionname, $inputparams = array(), $outputparams = array(), $documentation = '') {
        ...
        if (!empty($inputparams)) {
            // For portType operation input node.
            $input = $porttypeoperation->addChild('input');
            $input->addAttribute('message', 'tns:' . $functionname . self::IN);

            // For binding operation input node.
            $input = $bindingoperation->addChild('input');
            $soapbody = $input->addChild('x:soap:body');
            ...
            // Process message input nodes.
            $messagein = $this->nodedefinitions->addChild('message');
            $messagein->addAttribute('name', $functionname . self::IN);
            foreach ($inputparams as $name => $options) {
        ...
        if (!empty($outputparams)) {
            // For portType operation output node.
            $output = $porttypeoperation->addChild('output');
            $output->addAttribute('message', 'tns:' . $functionname . self::OUT);

            // For binding operation output node.
            $messageout = $this->nodedefinitions->addChild('message');
            $messageout->addAttribute('name', $functionname . self::OUT);
            foreach ($outputparams as $name => $options) {
                $part = $messageout->addChild('part');
                $part->addAttribute('name', $name);
                $part->addAttribute('type', $this->get_soap_type($options['type']));
            }

            // Process message output nodes.
            $output = $bindingoperation->addChild('output');
            $soapbody = $output->addChild('x:soap:body');
            $soapbody->addAttribute('use', 'encoded');
            $soapbody->addAttribute('encodingStyle', self::NS_SOAP_ENC);
            $soapbody->addAttribute('namespace', $this->namespace);
        }
