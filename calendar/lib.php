...
function calendar_get_events($tstart, $tend, $users, $groups, $courses, $withduration = true, $ignorehidden = true) {
    ...
    $subquery = "SELECT ev.modulename,
                   ...
                   FROM {event} ev
             INNER JOIN (
                        SELECT DISTINCT ecat.modulename,
                                        ecat.instance,
                                        ecat.eventtype
                                   FROM {event} ecat
                              LEFT JOIN {modules} mcat
                                     ON ecat.modulename = mcat.name
                                  WHERE (mcat.visible = 1 OR mcat.visible IS NULL)
                        ) cat
                     ON cat.modulename = ev.modulename
                        AND cat.instance = ev.instance
                        AND cat.eventtype = ev.eventtype
                  $subquerywhere
