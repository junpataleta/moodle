...
class activity_information implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output): stdClass {
        ...
        foreach ($this->completiondata->details as $key => $detail) {
            ...
            $detail->key = $key;
            // We'll show this status as completed if the we're not tracking completion for this user (e.g. a teacher) or the
            // user has completed and/or passed this completion rule.
            $detail->istrackeduser = $data->istrackeduser;
            $detail->statuscomplete = in_array($detail->status, [COMPLETION_COMPLETE, COMPLETION_COMPLETE_PASS])
                || !$detail->istrackeduser;
