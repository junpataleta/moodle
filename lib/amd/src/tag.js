        ...
        initManagePage: function() {
            ...
            $('body').on('click', 'a[data-action=addstandardtag]', function(e) {
                ...
                str.get_strings([
                    ...
                    modal.getRoot().on(ModalEvents.save, function(e) {
                        e.preventDefault();
                        var name = $.trim($('#id_tagslist').val());
                        if (name.length > 0) {
                            // Looks good. Submit the form.
                            $('#addtags_form').submit();
                        } else {
                            // Show validation error when the tag collection name is empty.
                            $('[data-region="tagslistinput"]').addClass('has-danger notifyproblem');
                            $('#id_tagslist_error_message').removeAttr('hidden');
                        }
                        return false;
                    });
        ...
        initManageCollectionsPage: function() {
            ...
            $('body').on('click', '.addtagcoll > a', function(e) {
                ...
                str.get_strings(keys).then(function(langStrings) {
                    ...
                    modal.getRoot().on(ModalEvents.save, function(e) {
                        e.preventDefault();
                        var name = $.trim($('#addtagcoll_form #addtagcoll_name').val());
                        if (name.length > 0) {
                            // Looks good. Submit the form.
                            $('#addtagcoll_form').submit();
                        } else {
                            // Show validation error when the tag collection name is empty.
                            $('[data-region="addtagcoll_nameinput"]').addClass('has-danger notifyproblem');
                            $('#id_addtagcoll_name_error_message').removeAttr('hidden');
                        }
                        return false;
                    });
