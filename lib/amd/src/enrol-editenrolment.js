 ...
 *
 * @module     core/enrol-editenrolment
...
define(['core/templates',
    ...
    function(Template, $, Str, Config, Notification, ModalFactory, ModalEvents, Fragment, Log) {
        ...
        EditEnrolment.prototype.initModal = function() {
            var triggerButtons = $('a.editenrollink');

            var strparams = [
                {key: 'editenrolment', component: 'enrol'}
            ];

            $.when(Str.get_strings(strparams)).then(function(strlist) {
                var modalTitle = strlist[0];

                return ModalFactory.create({
                    type: ModalFactory.types.SAVE_CANCEL,
                    title: modalTitle,
                    body: this.getBody(triggerButtons)
                }, triggerButtons).then(function(modal) {
                    this.modal = modal;
                    this.modal.setLarge();

                    // We want the reset the form every time it is opened.
                    this.modal.getRoot().on(ModalEvents.shown, function(e) {
                        var invoker = $(e.relatedTarget);
                        this.modal.setBody(this.getBody(invoker));
                    }.bind(this));
         ...
         * @private
         */
        EditEnrolment.prototype.submitForm = function(e) {
            e.preventDefault();
         ...
         * @param {Event} e Form submission event.
         */
        EditEnrolment.prototype.submitFormAjax = function(e) {
         ...
         * @return {Promise}
         */
        EditEnrolment.prototype.getBody = function(triggerButton) {
            var params = {
                'ueid': triggerButton.attr('rel')
            };
            Log.error(params);
            return Fragment.loadFragment('enrol', 'user_enrolment_form', this.contextid, params).fail(Notification.exception);
        };

        return /** @alias module:core/enrol-editenrolment */ {
