...
class xapi_test_statement_post extends base {
    ...
    /**
     * Compare if a given statement is similar to the one on the record. The information
     * stored in the logstore is not exactly a xAPI standard.
     *
    ...
    public function compare_statement(stdClass $statement): bool {
        ...
        if ($this->data['other']['verb']['id'] != $statement->verb->id) {
            return false;
        }
        // Check user.
        ...
        // Check object.
        if ($this->data['other']['object']['id'] != $statement->object->id) {
            return false;
        }
        return true;
    }
