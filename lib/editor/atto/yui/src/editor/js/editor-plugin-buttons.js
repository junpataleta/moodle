...
EditorPluginButtons.prototype = {
     ...
     *
     * A button that was pressed after last selection change that may need to be highlighted.
     *
    ...
    addButton: function(config) {
        ...
        if (config.tags) {
            ...
            this._buttonHandlers.push(
                // A selection change resets all of the button hightlights.
                host.on('atto:selectionchanged', this._handleButtonHighlight,
                   this, config.tags, config.buttonName, tagMatchRequiresAll),
                // Click action may change highlighing.
                this.toolbar.delegate('click', this._handleButtonHighlight, '.' + buttonClass,
                   this, config.tags, config.buttonName, tagMatchRequiresAll),
                // Keyboard input may change highlighting.
                host.editor.delegate('key',
                   Y.rbind(this._handleButtonHighlight, this, config.tags, config.buttonName, tagMatchRequiresAll),
                   'up:', CSS.EDITORWRAPPER)
            );
        }
    ...
    /**
     * Set appropriate highlighting for button.
     *
     * @method _handleButtonHighlight
     * @param {EventFacade} e
     * @param {string} tags List of tags to look for
     * @param {string} buttonName The name of button of which adjust highlighting
     * @param {boolean} tagMatchRequiresAll Whether all tags need to be matched
     * @private
     */
    _handleButtonHighlight: function(e, tags, buttonName, tagMatchRequiresAll) {
        var host = this.get('host'),
            selection = host.getSelection()[0],
            toggleHighlighting = tags && selection && selection.collapsed;

        // If selection changes, no button should show formating change.
        if (e.type === 'atto:selectionchanged') {
            this.activeButton = null;
        }

        // If a button is activated with a collapsed selection, toggle the highligthing.
        toggleHighlighting = toggleHighlighting &&
                  !!this.buttons[buttonName] &&
                  !!this.activeButton &&
                  this.buttons[buttonName].compareTo(this.activeButton);

        if (typeof this._highlightQueue[buttonName] !== 'undefined') {
            this._highlightQueue[buttonName].cancel();
        }

        // Async the highlighting.
        this._highlightQueue[buttonName] = Y.soon(Y.bind(function() {
            // Set the current highlighting value or if toggleHighlighting is set, the highlight after input.
            if (toggleHighlighting !== host.selectionFilterMatches(tags, host.getSelectedNodes(), tagMatchRequiresAll)) {
                this.highlightButtons(buttonName);
            } else {
                this.unHighlightButtons(buttonName);
            }
        }, this));
    },
    ...
    _normalizeCallback: function(config, inheritFrom) {
        ...
        config.callback = Y.rbind(this._callbackWrapper, this, config._callback, config.callbackArgs, config.buttonName);
     ...
     * @param {String} buttonName The name any button associated with this callback.
     * @return {Mixed} The value returned by the callback.
     * @private
     */
    _callbackWrapper: function(e, callback, callbackArgs, buttonName) {
        e.preventDefault();
        ...
        if (buttonName) {
            creatorButton = this.buttons[buttonName];
        }
        ...
        this.activeButton = creatorButton;
