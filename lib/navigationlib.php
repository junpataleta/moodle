...
class global_navigation extends navigation_node {
    ...
    protected function load_courses_enrolled() {
        ...
        if (count($courses) && $this->show_my_categories()) {
            ...
            // Get the course categories for these category IDs.
            require_once('coursecatlib.php');
            ...
            foreach ($mycoursecategories as $mycoursecat) {
                ...
                // No need to process empty paths. Shouldn't happen though.
                if (empty($pathcategoryids)) {
                    continue;
                }

                // Add each category in the path to the nav tree.
                $pathcategories = coursecat::get_many($pathcategoryids);
                foreach ($pathcategories as $coursecat) {
                    // No need to process categories that have already been added.
                    if ($this->rootnodes['mycourses']->find($coursecat->id, self::TYPE_MY_CATEGORY)) {
                        continue;
                    }
