 ...
 *   2. font - icon font markup is generated with the name of the icon mapped from the moodle icon name.
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class icon_system_font extends icon_system {

    /** @const FORKAWESOME Fork Awesome icon font name.  */
    const FORKAWESOME = 'forkawesome';

    /** @var array The icon map being used for this instance. */
    protected $map = [];

    /** @var string The icon font used. */
    protected $iconfont;

    /** @var \string[][] Mapping of Moodle core icons to font icons. */
    protected static $iconmaps = [
        self::FORKAWESOME => [
    ...
    /**
     * Fetches the default icon font used.
     *
     * @return string
     */
    public static function get_default_icon_font(): string {
        // Default to Fork Awesome.
        return self::FORKAWESOME;
    }

    public function get_icon_font(): string {
        global $PAGE;
        if (empty($this->iconfont)) {
            $this->iconfont = $PAGE->theme->get_icon_font_name();
        }
        return $this->iconfont;
    }

    /**
     * Checks that the icon font being used is valid.
     *
     * @param string $iconfontname
     * @return bool
     */
    public static function is_valid_icon_font(string $iconfontname): bool {
        return array_key_exists($iconfontname, self::$iconmaps);
    }

    /**
     * Get the core icon map.
     *
     * @return mixed
     */
    public function get_core_icon_map() {
        $iconfontname = $this->get_icon_font();

        return static::$iconmaps[$iconfontname];
    }

    public function get_amd_name() {
        return 'core/icon_system_font';
    }

    public function render_pix_icon(renderer_base $output, pix_icon $icon) {
        $iconfont = $this->get_icon_font();
        $subtype = 'pix_icon_' . $iconfont;
        if (!class_exists($subtype)) {
            $subtype = pix_icon_font::class;
        }
        ...
        return $output->render_from_template('core/pix_icon_' . $iconfont, $data);
    }
