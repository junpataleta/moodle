...
class core_enrol_externallib_testcase extends externallib_advanced_testcase {
    ...
    public function test_edit_user_enrolment() {
        ...
        $this->assertEmpty($data['formoutput']);

        // Invalid data with form.
        $data = core_enrol_external::edit_user_enrolment($course->id, $ueid, ENROL_USER_ACTIVE, $nowtime, $nowtime, true);
        $data = external_api::clean_returnvalue(core_enrol_external::edit_user_enrolment_returns(), $data);
        $this->assertFalse($data['result']);
        $this->assertNotEmpty($data['errors']);
        $this->assertNotEmpty($data['formoutput']);

        // Valid data.
        $nextmonth = clone($now);
        $nextmonth->add(new DateInterval('P1M'));
        $nextmonthtime = $nextmonth->getTimestamp();
        $data = core_enrol_external::edit_user_enrolment($course->id, $ueid, ENROL_USER_ACTIVE, $nowtime, $nextmonthtime, true);
        $data = external_api::clean_returnvalue(core_enrol_external::edit_user_enrolment_returns(), $data);
