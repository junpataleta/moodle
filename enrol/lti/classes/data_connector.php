...
class data_connector extends DataConnector {
    ...
    public function deleteToolConsumer($consumer) {
        ...
        $queries[] = "DELETE FROM {{$resourcelinksharekeytable}} sk
                            WHERE sk.resource_link_pk IN (
        ...
        $queries[] = "DELETE FROM {{$userresulttable}} u 
                            WHERE u.resource_link_pk IN (
                                    SELECT rl.resource_link_pk 
                                      FROM {{$resourcelinktable}} rl
                                     WHERE rl.consumer_pk = :consumer_pk
                            )";
        ...
        $updatewhere = "primary_resource_link_pk IN (
                                SELECT rl.resource_link_pk
                                  FROM {{$resourcelinktable}} rl
                                 WHERE rl.consumer_pk = :consumer_pk
                            )";
        $queries[] = $this->build_update_sql($resourcelinktable, $updatecolumns, $updatewhere);

        // Update any resource links for contexts in which this consumer is acting as a primary resource link.
        $updatewhere2 = "primary_resource_link_pk IN (
                                SELECT rl.resource_link_pk
                                  FROM {{$resourcelinktable}} rl
                            INNER JOIN {{$contexttable}} c 
                                    ON rl.context_pk = c.context_pk
        ...
        $queries[] = $this->build_update_sql($resourcelinktable, $updatecolumns, $updatewhere2);

        // Delete any resource links for contexts in this consumer.
        $queries[] = "DELETE FROM {{$resourcelinktable}} rl
                            WHERE rl.context_pk IN (
                                  SELECT c.context_pk
                                    FROM {{$contexttable}} c
                                   WHERE c.consumer_pk = :consumer_pk
                            )";
        ...
        foreach ($queries as $sql) {
            $DB->execute($sql, $deletecondition);
        }
    ...
    public function deleteContext($context) {
        ...
        $sql = "DELETE FROM {{$resourcelinksharekeytable}} sk
                      WHERE sk.resource_link_pk IN (
                                SELECT rl.resource_link_pk 
                                  FROM {{$resourcelinktable}} rl
        ...
        $sql = "DELETE FROM {{$userresulttable}} u 
                      WHERE u.resource_link_pk IN (
                                SELECT rl.resource_link_pk 
                                  FROM {{$resourcelinktable}} rl
                                 WHERE rl.context_pk = :context_pk    
    ...
    public function getUserResultSourcedIDsResourceLink($resourcelink, $localonly, $idscope) {
        ...
        $sql = "SELECT u.user_pk, u.lti_result_sourcedid, u.lti_user_id, u.created, u.updated
                  FROM {{$userresulttable}} AS u
            INNER JOIN {{$resourcelinktable}} AS rl
                    ON u.resource_link_pk = rl.resource_link_pk
                 WHERE (rl.resource_link_pk = :resource_link_pk AND rl.primary_resource_link_pk IS NULL)";

        if (!$localonly) {
            $sql .= " OR (rl.primary_resource_link_pk = :resource_link_pk2 AND share_approved = 1)";
    ...
    public function loadResourceLinkShareKey($sharekey) {
        ...
        if ($sharekeyrecord = $DB->get_record($resourcelinksharekeytable, ['share_key_id' => $sharekey->getId()], $fields)) {
            if ($sharekeyrecord->resource_link_pk == $sharekey->resourceLinkId) {
                ...
                $sharekey->expires = strtotime($sharekeyrecord->expires);
                return true;
            }
        }
    ...
    protected function build_tool_consumer_object($record, ToolConsumer $consumer) {
        ...
        if (!is_null($record->enable_from)) {
            $consumer->enableFrom = strtotime($record->enable_from);
        }
        $consumer->enableUntil = null;
        if (!is_null($record->enable_until)) {
            $consumer->enableUntil = strtotime($record->enable_until);
        }
        $consumer->lastAccess = null;
        if (!is_null($record->last_access)) {
            $consumer->lastAccess = strtotime($record->last_access);
        }
        $consumer->created = strtotime($record->created);
        $consumer->updated = strtotime($record->updated);
    }
}
