...
/**
 * IMS Enterprise unenrolment tests.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_imsenterprise_unenrol_testcase extends advanced_testcase {
    ...
    /**
     * @var $unenrol_constants - instance of enrol_imsenterprise_unenrol_behaviour to allow easy access to constants.
     */
    private static $unenrolconstants;

    public static function setUpBeforeClass() {
        self::$unenrolconstants = new enrol_imsenterprise_unenrol_behaviour();
            ...
            foreach ($users as $j => $u) {
                if (isset($linkmatrix[$i][$j])) {
                    ...
                    if (strlen($rolerecstatus) && strlen($roletype) && strlen($rolestatus)) {
                        $membership->member[] = (object)array(
    ...
    public function test_no_action_when_unenrol_disabled() {
        ...
        $coursemembership = $this->link_users_with_courses(
            ...
            array(
                array('01:1:1', '01:1:1'), // Course 1.
                array('01:1:1', '01:1:1'), // Course 2.
                array('::', '01:1:1'), // Course 3.
            )
        );
        ...
        $coursemembership = $this->link_users_with_courses(
            ...
            // Format of matrix elements: <roletype>:<role status>:<role recstatus>.
            array(
                array('01:1:3', '01:1:3'), // Course 1.
                array('::', '01:1:3'), // Course 2.
                array('::', '01:1:3'), // Course 3.
            )
        );
        ...
        // Enable unenrolment, and check that usesr are unenroled as expected.
        $this->imsplugin->set_config('imsunenrol', 1);
        $this->imsplugin->set_config('imsunenrolaction', $consts::ENROL_IMSENTERPRISE_REMOVE_ENROL_ONLY);
    ...
    public function test_existing_roles_and_enrolments_unaffected() {
        ...
        $this->imsplugin->set_config('imsunenrol', 1);
        $this->imsplugin->set_config('imsunenrolaction', $consts::ENROL_IMSENTERPRISE_REMOVE_ENROL_AND_ROLES);
        ...
        $enrolid = $DB->insert_record('enrol', (object) array('enrol' => 'imsenterprise',
            'courseid' => $course1->id, 'status' => 1, 'roleid' => 5
        ), true);

        $user1 = $this->getDataGenerator()->create_and_enrol($course1, 'student',
            array('idnumber' => 'UserIDNumber100'), $enrol = 'imsenterprise');
        ...
        $initialusernerolment = $DB->get_record('user_enrolments', array('userid' => $user1->id),
            '*', MUST_EXIST);
        $initialroleassigned = $DB->get_record('role_assignments', array('userid' => $user1->id),
            '*', MUST_EXIST);
        ...
        $coursemembership = $this->link_users_with_courses(
            ...
            array(
                array('::'), // Course 1.
                array('01:1:1'), // Course 2.
            )
        );

        $this->set_xml_file(array($user1), $courses, $coursemembership);
        $this->imsplugin->cron();

        $this->assertEquals(2, $DB->count_records('user_enrolments', array('userid' => $user1->id)));
        ...
        $this->assertEquals(1, $DB->count_records('user_enrolments', array('userid' => $user1->id)));
        $this->assertTrue($DB->record_exists('user_enrolments', array('id' => $initialusernerolment->id,
            'userid' => $initialusernerolment->userid)));
        $this->assertTrue($DB->record_exists('role_assignments', array('id' => $initialroleassigned->id,
            'userid' => $initialusernerolment->userid)));
    }
    ...
    public function test_disable_enrolments_only() {
        ...
        $this->imsplugin->set_config('imsunenrol', 1);
        $this->imsplugin->set_config('imsunenrolaction', $consts::ENROL_IMSENTERPRISE_DISABLE_ENROL_ONLY);
        ...
        // Capture DB ids.
        $dbuser = $DB->get_record('user', array('idnumber' => $users[0]->idnumber), '*', MUST_EXIST);

        $dbenrolment = $DB->get_record('user_enrolments',
            array('userid' => $dbuser->id, 'status' => ENROL_USER_ACTIVE),
            '*', MUST_EXIST
        );

        $dbrole = $DB->get_record('role_assignments', array('userid' => $dbuser->id), '*', MUST_EXIST);
        ...
        $this->assertEquals(1, $DB->count_records('user_enrolments',
            array('userid' => $dbuser->id, 'id' => $dbenrolment->id, 'status' => ENROL_USER_SUSPENDED)));

        $this->assertEquals(1, $DB->count_records('role_assignments',
            array('userid' => $dbuser->id, 'id' => $dbrole->id)));
    }
    ...
    public function test_disable_enrolments_and_remove_roles() {
        ...
        $this->imsplugin->set_config('imsunenrol', 1);
        $this->imsplugin->set_config('imsunenrolaction', $consts::ENROL_IMSENTERPRISE_DISABLE_ENROL_REMOVE_ROLES);
    ...
    /**
     * Enrolments alone are deleted for specified user
     */
    public function test_remove_enrolment_only() {
