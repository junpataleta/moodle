    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        if (!$placeholder) {
            ...
            if (!empty($options->questionslot)) {
                ...
                $questionnumber = html_writer::span(get_string('answercolon', 'qtype_numerical', ''), '', ['aria-hidden' => 'true']);
                // Screen reader-only text label that will link the answer input to the question number.
                $questionnumber .= html_writer::span(get_string('answerquestionx', 'question', $options->questionslot), 'sr-only');
            } else {
