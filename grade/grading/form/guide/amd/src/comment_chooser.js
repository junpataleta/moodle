...
define(['jquery', 'core/templates', 'core/notification', 'core/yui'], function ($, templates, notification) {
    ...
    return /** @alias module:gradingform_guide/comment_chooser */ {
         ...
         *
         * @param titleLabel The title for the header content of the dialog.
         ...
         * @param noLabel The text for the cancel button.
         */
        initialise: function (titleLabel, buttonId, remarkId, noLabel) {
            ...
            function generateCommentsChooser() {
                // Get list of frequently used comment options from a hidden input.
                var comments = $.parseJSON($('#frequently-used-comment-options').val());
                ...
                templates.render('gradingform_guide/comment_chooser', context)
                    .done(function (compiledSource) {
                        displayChooserDialog(compiledSource, comments);
                    })
            ...
            function displayChooserDialog(compiledSource, comments) {
                var cancelButtonId = 'comment-chooser-cancel';
                var cancelButton = '<button id="' + cancelButtonId + '">' + noLabel + '</button>';
                ...
                $.each(comments, function (index, comment) {
                    var commentOptionId = '#comment-option-' + comment.id;

                    // Delegate click event for the generated option link.
                    // Turn it's click event off before turning it on in order to fire the click event only once.
