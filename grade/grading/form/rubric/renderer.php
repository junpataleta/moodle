...
class gradingform_rubric_renderer extends plugin_renderer_base {
    ...
    public function criterion_template($mode, $options, $elementname = '{NAME}', $criterion = null, $levelsstr = '{LEVELS}', $value = null) {
        ...
        $levelstableparams = array(
            ...
            'aria-describedby' => '{NAME}-criteria-{CRITERION-id}-levels-table-desc'
        );
        ...
        $levelstabledescparams = array('id' => '{NAME}-criteria-{CRITERION-id}-levels-table-desc');
        $levelstabledesc = html_writer::div(get_string('levelsgroup', 'gradingform_rubric'), 'hidden', $levelstabledescparams);

        $criteriontemplate .= html_writer::tag('td', $levelstabledesc . $levelsstrtable, array('class' => $levelsclass));
        ...
        if ($displayremark) {
            ...
            $remarklabelparams = array('id' => '{NAME}-criteria-{CRITERION-id}-remark-desc');
            $remarklabel = html_writer::div($remarklabeltext, 'accesshide', $remarklabelparams);
            ...
            if ($mode == gradingform_rubric_controller::DISPLAY_EVAL) {
                ...
                $remarkparams = array(
                    ...
                    'cols' => '10', 'rows' => '5',
                    'aria-labelledby' => '{NAME}-criteria-{CRITERION-id}-remark-desc'
                );
    ...
    protected function rubric_template($mode, $options, $elementname, $criteriastr) {
        ...
        // Hidden rubric label.
        $rubricdescparams = array('id' => '{NAME}-criteria-desc');
        $rubricdesc = html_writer::div(get_string('rubric', 'gradingform_rubric'), 'hidden', $rubricdescparams);
        $rubrictemplate .= $rubricdesc;

        // Rubric table.
        $rubrictableparams = array(
            'class' => 'criteria',
            'id' => '{NAME}-criteria',
            'aria-describedby' => '{NAME}-criteria-desc');
