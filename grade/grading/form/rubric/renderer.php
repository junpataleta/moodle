...
class gradingform_rubric_renderer extends plugin_renderer_base {
    ...
    public function criterion_template($mode, $options, $elementname = '{NAME}', $criterion = null, $levelsstr = '{LEVELS}', $value = null) {
        ...
        if ($mode == gradingform_rubric_controller::DISPLAY_EDIT_FULL) {
            ...
            $descriptiontextareaparams = array(
                ...
                'aria-labelledby' => 'descriptionlabel-{CRITERION-id}',
                'cols' => '10', 'rows' => '5'
            );
            $description = html_writer::tag('textarea', s($criterion['description']), $descriptiontextareaparams);

            // Hidden description label.
            $descriptionlabelparams = array(
                'id' => 'descriptionlabel-{CRITERION-id}',
                'class' => 'accesshide'
            );
            $descriptionlabel = html_writer::label(get_string('criterion', 'gradingform_rubric', ''),
                                                   '{NAME}-criteria-{CRITERION-id}-description', false,
                                                   $descriptionlabelparams);
        } else {
            ...
            // No need for description label when not in edit mode.
            $descriptionlabel = '';
        }
        ...
        if ($mode != gradingform_rubric_controller::DISPLAY_EDIT_FULL &&
            ...
            $descriptiontdparams['aria-labelledby'] = 'descriptionlabel-{CRITERION-id}';
        }

        // Description cell.
        $criteriontemplate .= html_writer::tag('td', $description . $descriptionlabel, $descriptiontdparams);
        ...
        $levelstableparams = array('id' => '{NAME}-criteria-{CRITERION-id}-levels-table');
        ...
        $levelstablelabelparams = array('class' => 'accesshide');
        $levelstablelabel = html_writer::label(get_string('levelsgroup', 'gradingform_rubric'),
                                               '{NAME}-criteria-{CRITERION-id}-levels-table', false, $levelstablelabelparams);

        $criteriontemplate .= html_writer::tag('td', $levelstablelabel . $levelsstrtable, array('class' => $levelsclass));
        ...
        if ($displayremark) {
            ...
            $remarklabeltext = get_string('criterionremark', 'gradingform_rubric', s($criterion['description']));
            $remarklabelparams = array('class' => 'accesshide');
            $remarklabel = html_writer::label($remarklabeltext, '{NAME}-criteria-{CRITERION-id}-remark', false, $remarklabelparams);
            ...
            if ($mode == gradingform_rubric_controller::DISPLAY_EVAL) {
                ...
                $criteriontemplate .= html_writer::tag('td', $remarklabel . $input, array('class' => 'remark'));
                ...
                $criteriontemplate .= html_writer::tag('td', $remarklabel . s($currentremark), $remarkparams);
            }
        }
    ...
    public function level_template($mode, $options, $elementname = '{NAME}', $criterionid = '{CRITERION-id}', $level = null) {
        ...
        if ($mode == gradingform_rubric_controller::DISPLAY_EDIT_FULL) {
            $definitionparams = array(
                ...
                'aria-labelledby' => 'definitionlabel[{CRITERION-id}][{LEVEL-id}]',
                'cols' => '10', 'rows' => '4'
            );
            $definition = html_writer::tag('textarea', s($level['definition']), $definitionparams);

            // Hidden definition label.
            $definitionlabelparams = array(
                'id' => 'definitionlabel[{CRITERION-id}][{LEVEL-id}]',
                'class' => 'accesshide'
            );
            $definitionlabeltext = get_string('leveldefinition', 'gradingform_rubric', $levelindex);
            $definitionlabel = html_writer::label($definitionlabeltext,
                                                  '{NAME}-criteria-{CRITERION-id}-levels-{LEVEL-id}-definition', false,
                                                  $definitionlabelparams);

            $scorelabeltext = get_string('scoreinputforlevel', 'gradingform_rubric', $levelindex);
            $score = html_writer::label(
                $scorelabeltext,
            ...
            // No need for definition label when not in edit mode.
            $definitionlabel = '';
        }
        ...
        if ($mode == gradingform_rubric_controller::DISPLAY_EVAL) {
            ...
            $radioparams = array('role' => 'radio');
            $leveltemplate .= html_writer::div($input, 'radio', $radioparams);
        }
        ...
        $leveltemplate .= html_writer::div($definition . $definitionlabel, $definitionclass, $leveltemplateparams);
        $displayscore = true;
    ...
    protected function rubric_template($mode, $options, $elementname, $criteriastr) {
        ...
        // Hidden rubric label.
        $rubriclabelparams = array('class' => 'accesshide');
        $rubriclabel = html_writer::label(get_string('rubric', 'gradingform_rubric'), '{NAME}-criteria', false, $rubriclabelparams);
        $rubrictemplate .= $rubriclabel;
