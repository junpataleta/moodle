...
class api {
    ...
    public static function get_assigned_privacy_officer_roles() {
        ...
        if (!empty($configroleids)) {
            ...
            // Check if the config is different from the validated roles.
            if (array_diff($configroleids, $roleids)) {
                // If so, save the list of valid PO roles into the configuration.
                $newsetting = implode(',', $roleids);
                set_config('dporoles', $newsetting, 'tool_dataprivacy');
            }
        }
