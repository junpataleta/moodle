...
class tool_dataprivacy_api_testcase extends advanced_testcase {
    ...
    public function test_get_assigned_privacy_officer_roles() {
        ...
        // Confirm that the config is also now empty.
        $roleconfig = get_config('tool_dataprivacy', 'dporoles');
        $this->assertEmpty($roleconfig);
        ...
        // And it does not contain the editing teache role.
        $this->assertNotContains($editingteacherroleid, $roleids);

        // Check the configuration. It should have been fixed.
        $roleconfig = get_config('tool_dataprivacy', 'dporoles');
        $this->assertNotEquals($badconfig, $roleconfig);
        $this->assertEquals($managerroleid, $roleconfig);
    }
